...
GM:RegisterHUDSection("ammo", function(colors, config, cache)
	...
	if IsValid(weapon) and weapon.Base == "eternity_firearm_base" and weapon.Primary.ClipSize > -1 then
		local setting = GAMEMODE:GetSetting("hud_ammostyle")
		local str

		if setting == AMMOTYPE_SINGLE then
			local backup = 0
			local ammotype = weapon:GetFiremode():Get("AmmoGroup")
			...
			str = string.format("<font=eternity.ammo><ol>%s/%s", weapon:Clip1(), backup)
		elseif setting == AMMOTYPE_DOUBLE then
			str = string.format("<font=eternity.ammo><ol>%s/%s", weapon:Clip1(), weapon.Primary.ClipSize)
		end
...
GM:RegisterHUDSection("firemode", function(colors, config, cache)
	...
	if IsValid(weapon) and weapon.Base == "eternity_firearm_base" then
		local firemode = weapon:GetFiremode().Name
		local ammo = GAMEMODE:GetAmmo(weapon:GetAmmoType())
...
GM:RegisterHUDSection(nil, function(colors, config, cache)
	...
	if GAMEMODE.Flashbang - CurTime() > 2 then
		range = 0
	end
