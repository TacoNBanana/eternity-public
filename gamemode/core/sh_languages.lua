...
if SERVER then
	hook.Add("InitialCharacterSetup", "languages.InitialCharacterSetup", function(ply)
		local languages = ply:GetLanguages()

		if #languages > 0 then
			ply:SetActiveLanguage(languages[1])
		else
			ply:SetActiveLanguage(LANG_NONE)
		end
	end)
end