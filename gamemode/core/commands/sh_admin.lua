...
"Sandbox", "Persists the prop or effect you're looking at, making it unable to be edited and persist through restarts/map changes.")
...
console.AddCommand("rpa_ko", function(ply, targets)
	for _, v in pairs(targets) do
		...
		v:SendChat("NOTICE", string.format("%s knocked you out", PlayerName(ply)))
...
console.AddCommand("rpa_editstash", function(ply, target)
	...
	if not species.AllowStash then
		Feedback(ply, string.format("%s can't have a stash!", TargetName(targets, "RPName")), "ERROR")
