...
function CLASS:GetItemCount(classname)
	local amount = 0
...
function CLASS:IsLocked(ply)
	if self.EquipmentSlot then
		local item = self:GetItem(1, 1)

		if item and not item:CanUnequip(ply) then
			return true
		end
	end
...
if SERVER then
	...
	function CLASS:RemoveItem(item, unloading)
		if self.EquipmentSlot then
			item:PreUnequip(self:GetParent(), self.GridName, unloading)
		end
