...
if SERVER then
	...
	function ITEM:CanTransmit(ply)
		...
		if data.Preset then
			local channel = CHANNELS[data.Preset]

			if channel.Clear then
				return true
			end

			return ply:HasBiosignal(), "You don't have an active biosignal!"
		end
	...
	function ITEM:CanHearFrequency(ply, freq, key)
		...
		for _, v in pairs(self.ChannelData) do
			...
			if v.Preset then
				local channel = CHANNELS[v.Preset]

				if channel.Clear then
					clear = true
				else
					clear = tobool(ply:HasBiosignal())
				end
