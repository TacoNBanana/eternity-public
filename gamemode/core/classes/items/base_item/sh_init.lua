...
ITEM.OutlineColor 		= Color(255, 255, 255)
...
function ITEM:PreUnequip(ply, slot, unloading)
end
...
if CLIENT then
	function ITEM:GetTags()
		...
		if table.Count(self.Inventories) > 0 and self:CanInteractWithChild(LocalPlayer()) then
			table.insert(tags, "Container")
		end

		if self.Obscuring then
			table.insert(tags, "Obscuring")
		end
